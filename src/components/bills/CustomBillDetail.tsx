...
import { CustomBill } from '../../types';
import { format } from 'date-fns';

export default function CustomBillDetail() {
  const { billId } = useParams<{ billId: string }>();
  const navigate = useNavigate();
  const [bill, setBill] = useState<CustomBill | null>(null);
  ...
  const fetchBill = async (id: string) => {
    try {
      const billDoc = await getDoc(doc(db, 'customBills', id));
      if (billDoc.exists()) {
        ...
        setError('Custom bill not found');
      }
    } catch (error) {
      console.error('Error fetching custom bill:', error);
      setError('Failed to fetch custom bill');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-600 text-lg mb-4">{error || 'Custom bill not found'}</p>
          <button
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="border-b pb-4 mb-6">
            <h1 className="text-2xl font-bold text-gray-900">Custom Bill Details</h1>
